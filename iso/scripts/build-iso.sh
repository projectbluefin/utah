...
# Usage: build-iso.sh IMAGE OUTPUT_ISO [TITLE] [DEBUG]
set -euo pipefail
...
podman unshare bash -s -- "${LIVE_IMAGE}" "${OUTPUT_ISO}" "${TITLE}" "${LABEL}" "${WORK}" <<'ASSEMBLY'
set -euo pipefail
IMAGE="$1"
OUTPUT_ISO="$2"
TITLE="$3"
...
cleanup() {
    set +e
    podman image unmount "${IMAGE}" >/dev/null 2>&1
}
trap cleanup EXIT

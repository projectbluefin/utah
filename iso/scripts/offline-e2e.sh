#!/usr/bin/env bash
# Boot a Utah live ISO, install its embedded OCI payload with Fisherman, and
# boot the installed system without the ISO or a network.
...
# network. The installed VM has no network device at all. This is deliberate:
# a successful install must come from the containers-storage payload in the
# ISO, not from a registry pull that happens to be available in CI.
set -euo pipefail
...
cleanup() {
    local status=$?
    set +e
    # Keep the work directory, especially the failed disk image. A last
    # framebuffer capture makes a boot failure diagnosable even when the
    # failure happened before the phase-specific capture below.
    if [[ "${status}" -ne 0 ]]; then
...
echo "=== Phase 3/4: install the embedded payload with no registry access ==="
RECIPE="${WORK}/recipe.json"
python3 - "${RECIPE}" "${PAYLOAD_REF}" "${TARGET_IMGREF}" "${TEST_USER}" "${TEST_PASSWORD}" <<'PY'
import json
import pathlib
import sys

path, payload, target, user, password = sys.argv[1:]
recipe = {
    "disk": "/dev/vda",
    "filesystem": "btrfs",
...
echo "Preparing boot entries and a test-only graphical-session assertion..."
ssh_live "sudo TARGET_USER='${TEST_USER}' bash -s" <<'TARGET_SETUP'
set -euo pipefail
...
# The test disk is disposable. Auto-login lets the guest prove that a GNOME
# shell was created for the account Fisherman made, instead of treating a
# graphical target with only a greeter as a complete desktop boot.
...
# Forward boot status to the host serial log and make QEMU's removable-media
# fallback usable even if the firmware variable store is reset.
entries=()
while IFS= read -r -d '' entry; do
    entries+=("${entry}")
done < <(
    find "${root_mount}/boot" "${root_mount}/efi" "${esp_mount}" \
...
for entry in "${entries[@]}"; do
    ...
    grep -q 'console=ttyS0' "${entry}" || \
        sed -i 's|^options .*|& console=ttyS0,115200n8|' "${entry}"
...
echo "Utah identity and installed graphical assertion staged."
TARGET_SETUP
...
EOF
echo "PASS: Utah live ISO booted, installed Fisherman offline, and the installed graphical system booted."
echo "Evidence: ${WORK}"
